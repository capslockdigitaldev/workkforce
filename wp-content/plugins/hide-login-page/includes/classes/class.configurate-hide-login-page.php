<?php
	...
	class WHLP_ConfigHideLoginPage extends Wbcr_FactoryClearfy206_Configurate {
		...
		{
			return $this->disable_wp_admin && $this->disable_wp_login && $this->login_path && WbcrFactoryClearfy206_Helpers::strContains(rawurldecode($_SERVER['REQUEST_URI']), '/wp-admin/');
		}
		...
		public function init()
		{
			...
			if( !empty($this->login_path) && isset($_GET['wbcr_hlp_login_recovery']) ) {
				...
				$plugin_recovery_code = $this->getPopulateOption('login_recovery_code');
		...
		public function wpLoaded()
		{
			...
			if( $pagenow === 'wp-login.php' && $request['path'] !== WbcrFactoryClearfy206_Helpers::userTrailingslashit($request['path']) && get_option('permalink_structure') ) {
				$query_string = !empty($_SERVER['QUERY_STRING']) ? '?' . $_SERVER['QUERY_STRING'] : '';

				wp_safe_redirect(WbcrFactoryClearfy206_Helpers::userTrailingslashit($this->login_path) . $query_string);
				die();
				...
				if( $referer && WbcrFactoryClearfy206_Helpers::strContains($referer, 'wp-activate.php') && $parse_referer && !empty($parse_referer['query']) ) {
				...
				if( !$this->disable_wp_login || $new_login_redirect ) {
					...
					if( WbcrFactoryClearfy206_Helpers::strContains($_SERVER['REQUEST_URI'], 'wp-signup') ) {
						$redirect_uri = add_query_arg(array(
							'action' => 'register'
						), $redirect_uri);
					}
		...
		{
			if( $this->getPopulateOption('wpadmin_and_login_access_error') == "redirect_to" || $this->isAdminWhenThisIsBlocked()
			) {
				$access_redirect_to = $this->getPopulateOption('wpadmin_and_login_access_redirect', site_url() . '/404');
				wp_safe_redirect($access_redirect_to);
				die();
			} else if( $this->getPopulateOption('wpadmin_and_login_access_error') == "forbidden" && function_exists('status_header') ) {
				status_header('403');
				nocache_headers();
				die();
			} else {
				WbcrFactoryClearfy206_Helpers::setError404();
			}
		}
		...
		{
			return $value = str_replace('wp-login.php', WbcrFactoryClearfy206_Helpers::userTrailingslashit($this->login_path), $value);
		}

		public function newLoginUrl($scheme = null)
		{
			if( WbcrFactoryClearfy206_Helpers::isPermalink() ) {
				return WbcrFactoryClearfy206_Helpers::userTrailingslashit(home_url('/', $scheme) . $this->login_path);
			} else {
