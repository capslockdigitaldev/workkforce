            ...
            <div class="comment-content">
			    ...
			    <?php if (comments_open()) : ?>
                    <ul class="comment-options">
	                    <?php comment_reply_link(array_merge( $args, array('depth' => $depth, 'before' => '<li>', 'after' => '</li>', 'reply_text' => '<i class="fas fa-reply"></i> ' . esc_html__('Reply', 'pixiefreak'), 'max_depth' => $args['max_depth']))) ?>

                        <?php if (current_user_can('edit_posts')): ?>
                            <li>
                                <a href="<?php echo admin_url('comment.php?action=cdc&c='.$commentId); ?>">
                                    <i class="fas fa-trash"></i>
								    <?php esc_html_e('Delete', 'pixiefreak'); ?>
                                </a>
                            </li>
                            <li>
                                <a href="<?php echo admin_url('comment.php?action=cdc&dt=spam&c='.$commentId); ?>">
                                    <i class="fas fa-ban"></i>
								    <?php esc_html_e('Spam', 'pixiefreak'); ?>
                                </a>
                            </li>
					    <?php endif; ?>
                    </ul>
