...
if ( ! function_exists( 'ihs_generate_translated_msgs' ) ) {
	...
	function ihs_generate_translated_msgs() {
		...
		$ihs_otp_send_otp_text = ( get_option( 'ihs_otp_send_otp_text' ) ) ? get_option( 'ihs_otp_send_otp_text' ) : 'Send OTP';
		$ihs_otp_verify_text = ( get_option( 'ihs_otp_verify_text' ) ) ? get_option( 'ihs_otp_verify_text' ) : 'Verify OTP';
		$ihs_otp_resend_otp_text = ( get_option( 'ihs_otp_resend_otp_text' ) ) ? get_option( 'ihs_otp_resend_otp_text' ) : 'Resend OTP';
		$ihs_otp_required_text = ( get_option( 'ihs_otp_required_text' ) ) ? get_option( 'ihs_otp_required_text' ) : 'OTP( required )';
		$ihs_otp_reset_pwd_via_otp = ( get_option( 'ihs_otp_reset_pwd_via_otp' ) ) ? get_option( 'ihs_otp_reset_pwd_via_otp' ) : 'Reset Password via OTP';
		$ihs_otp_pls_verify_otp_first_text = ( get_option( 'ihs_otp_pls_verify_otp_first_text' ) ) ? get_option( 'ihs_otp_pls_verify_otp_first_text' ) : 'Please verify OTP first';
		$ihs_otp_pls_enter_otp_text = ( get_option( 'ihs_otp_pls_enter_otp_text' ) ) ? get_option( 'ihs_otp_pls_enter_otp_text' ) : 'Please enter OTP first';
		$ihs_otp_otp_enter_incorrect_text = ( get_option( 'ihs_otp_otp_enter_incorrect_text' ) ) ? get_option( 'ihs_otp_otp_enter_incorrect_text' ) : 'OTP Entered is Incorrect';
		$ihs_otp_otp_sent_to_mobile_text = ( get_option( 'ihs_otp_otp_sent_to_mobile_text' ) ) ? get_option( 'ihs_otp_otp_sent_to_mobile_text' ) : 'OTP sent to your mobile';
		$ihs_otp_sending_otp_text = ( get_option( 'ihs_otp_sending_otp_text' ) ) ? get_option( 'ihs_otp_sending_otp_text' ) : 'Sending OTP...';

		$ihs_wpml_messages = array(
			'mobile_box_label'     => __('Mobile Number with Country Code (required)', 'orion-sms-orion-sms-otp-verification'),
			'otp_required' => __( $ihs_otp_required_text, 'orion-sms-orion-sms-otp-verification'),
			'send_otp'       => __( $ihs_otp_send_otp_text, 'orion-sms-orion-sms-otp-verification'),
			'otp_verify_label' => __( 'Enter Verification Code sent', 'orion-sms-orion-sms-otp-verification'),
			'verify_otp'      => __( $ihs_otp_verify_text, 'orion-sms-orion-sms-otp-verification'),
			'resend_otp'      => __( $ihs_otp_resend_otp_text, 'orion-sms-orion-sms-otp-verification'),
			'reset_pwd_via_otp' => __( $ihs_otp_reset_pwd_via_otp, 'orion-sms-orion-sms-otp-verification'),
			...
			'sending_otp'  => __( $ihs_otp_sending_otp_text, 'orion-sms-orion-sms-otp-verification'),
			...
			'alerts'         => array(
				'pls_verify_otp_first' => __( $ihs_otp_pls_verify_otp_first_text, 'orion-sms-orion-sms-otp-verification' ),
				'pls_enter_req_field' => __( 'Please Enter the required fields', 'orion-sms-orion-sms-otp-verification' ),
				'pls_enter_otp' => __( $ihs_otp_pls_enter_otp_text, 'orion-sms-orion-sms-otp-verification' ),
				'thx_for_verification' => __( 'Thank you for verification', 'orion-sms-orion-sms-otp-verification' ),
				'otp_enter_incorrect' => __( $ihs_otp_otp_enter_incorrect_text, 'orion-sms-orion-sms-otp-verification' ),
				'tried_too_many_times' => __( 'You have tried too many times', 'orion-sms-orion-sms-otp-verification' ),
				'otp_sent_to_mobile' => __( $ihs_otp_otp_sent_to_mobile_text, 'orion-sms-orion-sms-otp-verification' ),
...
if ( ! function_exists( 'ihs_otp_enqueue_scripts' ) ) {
	...
	function ihs_otp_enqueue_scripts( $hook ) {
		...
		// Create an empty array to store selector information.
		...
		$resend_otp_timer = ( get_option( 'ihs_resend_otp_timer' ) ) ? get_option( 'ihs_resend_otp_timer' ) : 90;
		...
		if ( class_exists( 'WooCommerce' ) && is_checkout() ) {
			...
			$selector_array[] = array(
				'form_selector'       => trim( 'form.checkout.woocommerce-checkout' ),
...
if ( ! function_exists( 'ihs_otp_enqueue_admin_scripts' ) ) {
	...
	function ihs_otp_enqueue_admin_scripts( $hook ) {
		...
		|| 'orion-otp_page_ihs_otp_plugin_woocommerce_settings_page' === $hook ) {
			wp_enqueue_style( 'ihs_otp_admin_font_awesome', '//use.fontawesome.com/releases/v5.8.1/css/all.css' );
...
if ( ! function_exists( 'ihs_send_otp_via_transactional_route' ) ) {
	function ihs_send_otp_via_transactional_route (
		...
		include_once 'inc/unicode-file.php';
...
if ( ! function_exists( 'ihs_otp_send_new_pass' ) ) {
	...
	function ihs_otp_send_new_pass() {
		...
		} else if ( 'No' === $is_saved_with_country_code ){
...
if ( ! function_exists( 'ihs_reset_password_sms_by_twilio' ) ) {
	...
	function ihs_reset_password_sms_by_twilio( $message, $phone, $country_code ) {
		...
		require_once( IHS_OTP_PATH . 'vendor/twilio-php-master/Twilio/index.php' );
		...
		return $my_ihs_class->ihs_send_msg_using_twilio( $sid, $token, $mob_no_with_country_code, $twilio_mob_no, $message );
