...
if ( ! function_exists( 'ihs_send_order_sms_pending' ) ) {
	...
	function ihs_send_order_sms_pending( $order_id ) {
		...
		if ( isset($order_pending_arr['admin']) && $order_pending_arr['admin'] && ! empty( $data['message'] && $admin_mob_no ) ) {
		...
		if ( isset($order_pending_arr['customer']) && $order_pending_arr['customer'] && ! empty( $data['message'] ) && ! empty( $data['customer_phone'] ) ) {
...
if ( ! function_exists( 'ihs_send_order_sms' ) ) {
	 ...
	 * @param $phone
	 */
	function ihs_send_order_sms( $message, $phone, $country = '+91' ) {
	    
		$api_used =  ( get_option( 'ihs_api_type' ) ) ? get_option( 'ihs_api_type' ) : 'otp';
		// If twilio api used
		
		if ( 'twilio' === $api_used ) {
			ihs_send_order_sms_by_twilio( $message, $phone, $country );

		} else if ( 'msg91' === $api_used ) {
			ihs_send_order_sms_by_msg91( $message, $phone, $country );
		}
	}
}
...
if ( ! function_exists( 'ihs_send_order_sms_by_twilio' ) ) {
	 ...
	 * @return bool
	 */
	function ihs_send_order_sms_by_twilio( $message, $phone, $country ) {
	...
	// Your Account SID and Auth Token from twilio.com/console
		...
		//$country_code = get_option( 'ihs_otp_woo_country_code' );
		$mob_no_with_country_code = '+' . $country . $phone;
